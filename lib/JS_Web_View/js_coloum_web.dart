...
import '../database/coloum_chart_helper.dart';// Updated import for week data
...
class ColumnChartScreen extends StatelessWidget {
...
// pieChartData(parsePieChartData(json.decode(storedData)));

String parsePieChartData(Map<String, dynamic> jsonResponse) {
  List<dynamic> pieData = [];
  jsonResponse['data'].forEach((key, value) {
    if (key.endsWith('_[kW]')) {
      double total = (value as List).fold(0, (sum, item) => sum + (item is int ? item.toDouble() : item / 1000));
      pieData.add({'name': key.replaceAll('_[kW]', ''), 'y': total});
    }
  });

  return json.encode({'series': [{'name': 'Energy', 'colorByPoint': true, 'data': pieData}]});
}